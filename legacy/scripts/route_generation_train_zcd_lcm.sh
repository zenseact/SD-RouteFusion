#!/bin/bash
#SBATCH --job-name route_generation_zcd_lcm_train
#SBATCH --nodes 1
#SBATCH --gpus 0
#SBATCH --cpus-per-task 24
#SBATCH --time 10-00:00:00
#SBATCH --output /workspaces/%u/slurm/%j_train_lcm.out
#SBATCH -p zprodcpu
#SBATCH --no-requeue
#

# Define the output path as a variable
OUTPUT_PATH="/staging/dl_tt/path_prediction/data/all_data/processed_osm_lcm/zcd_osm_lcm_train"
OSM_CACHE_PATH="/staging/dl_tt/path_prediction/data/osm_cache"
MAP_QUERY_MAX_L1=2000
...
singularity exec --nv \
   ...
   --pwd /app \
    ...
    python3.11 main.py --input=/staging/dl_madmaps/data/dl2_hp_train_default --map_query_max_l1=$MAP_QUERY_MAX_L1 --output=$OUTPUT_PATH --dataset=ZCD --location_source=lcm --cache_location=$OSM_CACHE_PATH --workers=24
